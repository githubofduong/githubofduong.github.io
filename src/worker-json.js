...
"use strict";
    var at,
        ...
        error = function (m, p) {
            ...
            throw {
                name:    'InvalidSchema', //'SyntaxError'
                message: m,
                at:      p,
                text:    text
            };
        },
        findEndIndex = function(c, len) {
            var i = 0,
                currentChar = '',
                lastChar = '',
        ...
        validateResources = function(className, str) {
            if (!classNameList.length) { return true; }
            var el, subEl,
                classObj, attrObj;
            for (el in classList) {
    ...
    return function(text_json, dataModel) {
        ...
        // var classList = [];
        for (var o in dataModel) {
            var obj = dataModel[o];
            // name = dataModel[o].class;
            // attr = dataModel[o].attributes;
            if (obj.hasOwnProperty('class') && obj.hasOwnProperty('attributes')) {
                classList.push(obj);
                classNameList.push(obj.class);
            }
        }
        
        // postition of first non-blank char
        at = text.search(/[^\s]/);

    /**** not begin with array format ****/
        if (!Array.isArray(object_json)) {
            // console.log(at);
            error('Invalid schema: Expected to BEGIN with an ARRAY, please START with "[ ]"', at+1);    
        }

        // count elements of the array
        var arrLen = object_json.length;
        // array is empty
        if (!arrLen) {
            error('Invalid schema: Array is EMPTY, please ADD "{ }" to this array to have at least an OBJECT element', at+1);
        }

        // loop through each array's element
        var arrEl; // as a counter to iterate
        for (arrEl in object_json) {

            // access each object element
            var currentArrEl = object_json[arrEl];

            // check the type of each element
            if (Array.isArray(currentArrEl)) { // type: array
                error('Invalid schema: Element MUST BE of type OBJECT, please REPLACE "[]" with "{ }"', text.indexOf("[", at+1));
            } else if (typeof currentArrEl != "object") { // type: string, number
                var invalidEl;
                if (typeof currentArrEl == 'string') {
                    invalidEl = 'string "' +currentArrEl+ '"';
                } else {
                    invalidEl = 'number ' +currentArrEl+ '';
                }
                error('Invalid schema: Element MUST BE of type OBJECT, please REPLACE '+invalidEl+' with "[]"', text.indexOf(currentArrEl, at+1));
            }

            // element passed object type check

            var propertyList = Object.keys(currentArrEl); // string array of properties
            // object has no property (empty object)
            if (!propertyList.length) {
                error('Invalid schema: OBJECT is EMPTY, please ADD PROPERTY "class" to this object with FORMAT   "class": "<className>"', text.indexOf("{", at+1));
            }


            // object has at least a property (NON EMPTY)
            // main goal: remove properties which are not either "class" nor "permission"
            var eachProperty,
                classProperty = false,
                permissionProperty = false;
            for (eachProperty in propertyList) {

                // access a property object
                var currentProperty = propertyList[eachProperty];

                // currently access property is 'class'
                if (!classProperty && currentProperty == "class") {
                    // mark that property when the object has it
                    classProperty = true;
                } else if (!permissionProperty && currentProperty == "permission"){ // property is 'permission'
                    // mark it as well
                    permissionProperty = true;
                } else { // other properties are to be removed
                    error("Invalid schema: Property \"" +currentProperty+ "\" is NOT NEEDED, please REMOVE it and its corresponding value", text.indexOf(currentProperty, at+1));
                }
            }
            ...
            if (!Array.isArray(permissionValue)) { // not an array
                ...
                error('Invalid schema: Property "permission" is NOT ARRAY, please REPLACE ' +invalidType+ ' with "[]"', text.indexOf('permission', at+1));
            }
            // check elements of array "permission"
            var permLength = permissionValue.length,
                beginPermArrIndex = text.indexOf("[", at+1); // index of '[' of property 'permission'

            // array "permission" has no element
            if (!permLength) {
                error('Invalid schema: Property "permission" is an EMPTY ARRAY, please ADD "{ }" to the array to have at least an OBJECT element', beginPermArrIndex);
            } else { // at least an element in the array 'permission'
                var p_i = 0,
                ...
                for (; p_i < permLength; ++p_i) {
                    ...
                    // currently accessing object
                    p_currentObject = permissionValue[p_i];

                    // check type of each element of array 'permission'
                    if (Array.isArray(p_currentObject)) { // an array is invalid
                        error('Invalid schema: Element of array "permission" must be OBJECT, please REPLACE "[]" with "{ }"', text.indexOf('[', p_at+1));
                    } else if (typeof p_currentObject != "object") { // a string or number is also invalid
                        var invalidType;
                        if (typeof p_currentObject == 'string') {
                            invalidType = 'string "' +p_currentObject+ '"';
                        } else {
                            invalidType = 'number ' +p_currentObject;
                        }
                        error('Invalid schema: Element of array "permission" must be OBJECT, please REPLACE  ' +invalidType+ ' with "{ }"', text.indexOf(p_currentObject, p_at+1));
                    }

                    // elemnt is an object
                    // remove properties which are not needed
                    ...
                    // after removing all properties which are not needed
